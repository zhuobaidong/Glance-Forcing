...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        before_block_callback: Optional[callable] = None,
    ) -> torch.Tensor:
        """
        ...
        for block_index, current_num_frames in enumerate(tqdm.tqdm(all_num_frames)):
            ...
            for index, current_timestep in enumerate(current_denoising_list):
                # === 核心修改：在每个 step 开始前，触发回调函数并传入当前的 step 索引 (index) ===
                if before_block_callback is not None:
                    before_block_callback(index)
                ...
                print(f"Block {block_index} | Step {index} | current timestep: {timestep.tolist()}")
